...
Reading now, I feel like the Results could be edited to make the supporting cast (most of the other figures) feel more like supporting cast. For example, I like the idea of combining \fref{Ont_testing} and \fref{Ont_calibration_testify} (although it seems like we could do that just with panels?).
...
\david{Do we mean ``dynamical noise'' or ``demographic noise'' or ``observation noise'', ... ??  The term ``dynamical noise'' may seem generic, but it seems to have a specific meaning.}
\jd{I'm curious about this. I like the idea of using “dynamical noise” to mean what I used to call “process error” (broader than “demographic noise”), because process error seems like such a bad name for it.}
...
\ben{DE/JD/WZ: if we can get estimated-parameter table (estparmtab) and literature parameter table (litparmtab; \cref{tab:litparm}) working and properly cross-referenced, then we don't need the information in Mike's comments above. We may need to do some archaeology: were these in the original report to PHAC?}
...
\caption{Parameter estimates for base model calibration. \ben{JD/DE: should some parameters (change in transmission etc.) be back-transformed? Are parameter names comprehensible?  Should we add confidence intervals/std errs? We should explain that dispersion model is converging to Poisson; do we need to justify/explain the very high value of $\zeta$ (phenom het)?}\david{We definitely need to comment on
...
\caption{Calibration of positive tests  \ben{is there a nice way to combine Figures 6 and 7? Maybe a two-y-axis plot?}\david{two $y$ axes seems worth trying}}
...
\begin{itemize}
\item find and clean up \code{estparmtab}/\code{litparmtab} (DE, WZ?)
