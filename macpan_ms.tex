...
For example, specifying \code{log\_beta0 = -1} would specify that the baseline transmission parameter $\beta_0$ should be calibrated on the log scale (ensuring that the value of $\beta_0$ is always positive) using a starting value of -1 on the log scale (i.e., initial $\beta_0 = \exp(-1)$). 
Similarly, specifying \code{logit\_nonhosp\_mort = -0.2} would specify that the value of \code{nonhosp\_mort} (the proportion of people leaving the severely infected compartment who die at that moment) is fitted on a logit (log-odds) scale (ensuring that it is bounded between 0 and 1), using a starting value of $\beta_0 = \textrm{logit}^{-1}(-0.2) = 0.45$.
