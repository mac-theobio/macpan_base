...
Both models were used to forecast 30 days ahead. 
...
\item Bt and Rt plot? Even if they are a bit redundant, they are a bit different imo. If we include it, we need to make the plot and need a paragraph in the results. 
\item find and clean up \code{estparmtab}/\code{litparmtab} (done)
\item clean up Tables 1 and 2 (WZ? WZ: Combine it into a single table)
\item should we include fits to simulations or not? (Now sure what this means)
