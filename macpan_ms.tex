...
\mike{Columns 2j are the mobility intercepts and 2j+1 are the mobility slopes right? If yes, then this description is good.}
...
Specifying \code{logit\_nonhosp\_mort = -0.2} would specify that the value of \code{nonhosp\_mort} (the fraction of non-hospitalized mortality\david{is that the case fatality proportion outside hospital, or the proportion of deaths that occur outside hospital? I presume the former but it sounds like the latter} \mike{It is flowing from Is to D. So it is the latter.}) should be fitted on a logit, or log-odds, scale, ensuring that it is bounded between 0 and 1, and using a starting value of $\beta_0 = \textrm{logit}^{-1}(-0.2) = 0.45$.
