...
\begin{itemize}
\item base model is calibrating to new cases and death; testify is calibrating to positive test only
\item consistency of model (death forecast looks bad because the mort parameters are not time-varying), we can drop death and only calibrate to cases and this is now comparable with testify
\item \ben{re: the above two points, I'm not sure what the best answer is that's also consistent with doing as little work as possible :-) (1) fitting to multiple streams is supposedly one of our features (2) fitting to cases and death but not allowing breakpoints on death is supposedly giving us terrible-looking answers. I don't mind the base model and testify model doing different things}
